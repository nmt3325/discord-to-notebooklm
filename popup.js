(() => {
  ...
  async function findDiscordTab() {
    // CRITICAL FIX: Search ALL Discord tabs, not just active one
    const tabs = await chrome.tabs.query({ url: "https://discord.com/channels/*" });
    if (tabs.length) {
      state.tab = tabs[0];
      return true;
    }
    // Fallback: check if active tab is Discord
  ...
  async function tryGetTokenFromDiscordTab() {
    if (!state.tab?.id) return null;
    try {
      const result = await chrome.tabs.sendMessage(state.tab.id, { type: "D2N_GET_TOKEN" });
      return result?.token || null;
    } catch { return null; }
  }

  async function discoverDom() {
    const found = await findDiscordTab();
    if (!found) {
      setStatus("warn", "Discordタブが見つかりません", "Discord Web でチャンネルを開いてから拡張を再読込してください。");
      sourceHint.textContent = "Discordタブを開いて「再読込」してください。";
      setEnabled(false);
      return;
    }
    ...
    try {
      ...
      sourceHint.textContent = "再読込またはトークン入力を試してください。";
      setEnabled(false);
    }
  }
  ...
  async function applyToken(token) {
    ...
    try {
      ...
      state.user = user; state.useApi = true; state.mode = 'api';
      ...
      state.token = null; state.user = null; state.useApi = false; state.mode = 'none';
      tokenStatus.textContent = `❌ ${err.message}`;
      tokenStatus.classList.remove("loading"); tokenStatus.classList.add("error");
  ...
  tokenLoadButton.addEventListener("click", async () => {
    try {
      tokenLoadButton.disabled = true; tokenClearButton.disabled = true;
      await applyToken(tokenInput.value);
      await refreshAll();
    } catch {} finally {
      tokenLoadButton.disabled = false; tokenClearButton.disabled = false;
    }
  });

  tokenClearButton.addEventListener("click", async () => {
    state.token = null; state.user = null; state.useApi = false; state.mode = 'none';
    state.guilds = []; state.channels = []; state.selectedGuildId = null; state.selectedChannelId = null;
  ...
  tokenInput.addEventListener("keydown", async (e) => {
    if (e.key === "Enter") { e.preventDefault(); await applyToken(tokenInput.value); await refreshAll(); }
  });
  ...
  async function refreshAll() {
    ...
    // If we have a token, prioritize API mode
    if (state.useApi && state.token) {
      try {
        ...
        sourceHint.textContent = "DOM取得を試します…";
      }
    }
    ...
    // If DOM also failed but we have a Discord tab, try to auto-detect token
    if (!state.useApi && state.tab?.id) {
      const autoToken = await tryGetTokenFromDiscordTab();
      if (autoToken) {
        tokenInput.value = autoToken;
        try {
          await applyToken(autoToken);
          await refreshAll();
          return;
        } catch {}
      }
  ...
  async function onGuildChange() {
    ...
    if (state.useApi && state.token && state.selectedGuildId) {
      ...
      } catch (err) { setStatus("error", "チャンネル取得失敗", err.message); }
    }
  ...
  async function exportChannel(mode) {
    ...
    state.busy = true; setEnabled(false);
    try {
  ...
  // === Init ===
  async function init() {
    // 1. Restore saved token first (fastest)
    const stored = await chrome.storage.local.get("discordToken");
    if (stored.discordToken) {
      tokenInput.value = stored.discordToken;
      try { await applyToken(stored.discordToken); await refreshAll(); return; } catch {}
    }

    // 2. Try to find a Discord tab and use DOM mode
    if (await findDiscordTab()) {
      // Try to auto-detect token from Discord's localStorage first
      const autoToken = await tryGetTokenFromDiscordTab();
      if (autoToken) {
        tokenInput.value = autoToken;
        try { await applyToken(autoToken); await refreshAll(); return; } catch {}
      }
      // Fallback to DOM
      await discoverDom();
      return;
    ...
    // 3. Nothing found - show instructions
    setStatus("", "準備中", "Discord Web でチャンネルを開くか、トークンを入力してください。");
