(() => {
  ...
  function bestLabel(el, fb) { return [el?.getAttribute("aria-label"), el?.getAttribute("data-dnd-name"), el?.getAttribute("title"), el?.querySelector("img[alt]")?.getAttribute("alt"), el?.textContent].map(D.cleanText).find((v) => v && v.length < 120) || fb; }
  ...
  function discoverContext() {
    ...
    for (const a of anchors) {
      ...
      const cg = parts[1], cc = parts[2];
      if (!cg || cg === "@me") continue;
      if (!serversById.has(cg)) serversById.set(cg, { id: cg, name: bestLabel(a.closest('[data-list-item-id^="guildsnav"], [aria-label]') || a, `Server ${cg.slice(-4)}`), href: `/channels/${cg}` });
      if (cg === guildId && cc && numeric.test(cc)) channelsById.set(cc, { id: cc, name: `# ${bestLabel(a, `ch-${cc.slice(-4)}`).replace(/^Text Channel\s*/i, "").replace(/^テキストチャンネル\s*/i, "").replace(/^#\s*/, "")}`, href });
    }
    const heading = document.querySelector('main h1, [class*="titleWrapper"] h1, [class*="title"] [class*="name"]');
    const cn = D.cleanText(heading?.textContent || channelsById.get(channelId)?.name || "current-channel").replace(/^#\s*/, "");
    ...
    if (channelId && numeric.test(channelId) && !channelsById.has(channelId)) channelsById.set(channelId, { id: channelId, name: `# ${cn}`, href: location.pathname });
  ...
  function messageElements(root = document) { const p = [...root.querySelectorAll('li[id^="chat-messages-"], [data-list-item-id^="chat-messages"]')]; return p.length ? p : [...root.querySelectorAll('[id^="message-content-"]')].map((i) => i.closest("li, article, div")).filter(Boolean); }
  ...
  function extractMessagesFromDocument(root = document) {
    ...
    for (const el of messageElements(root)) {
      ...
      const ae = el.querySelector('[class*="username"], h3 span[class*="username"], [data-text-variant*="text-md"]');
      ...
      const atts = [...el.querySelectorAll('a[href*="cdn.discordapp.com"], a[href*="media.discordapp.net"], a[class*="fileNameLink"]')].map((l) => l.href).filter(Boolean);
      const reply = D.cleanText(el.querySelector('[class*="repliedMessage"], [class*="replying"]')?.textContent || "");
  ...
  async function exportHistory(options = {}) {
    ...
    for (let r = 0; r < 300; r++) {
      ...
      const old = srt[0]; ov.textContent = `${Math.min(srt.length, max).toLocaleString()}件をDOMから読み込み中…`;
    ...
    if (!msgs.length) throw new Error("画面からメッセージを検出できませんでした。");
    return msgs;
  }
  ...
  if (globalThis.__D2N_ENABLE_TEST_HOOKS__) globalThis.__D2N_TEST__ = { discoverContext, extractMessagesFromDocument, markdownFromMessages };
})();